...
function getMatchableManifestForPaths(
  ...
  return paths.map(([normalizedRoutePath, absoluteRoute, node]) => {
    // For generated routes from generateStaticParams, we need to use the original
    // dynamic route's pattern for proper param extraction
    const fileForRegex = node.destinationContextKey || node.contextKey;

    let matcher: ExpoRouterServerManifestV1Route;

    // For routes generated by `generateStaticParams()`, create the regex from the original dynamic
    // route pattern to preserve the namedRegex and routeKeys from the dynamic route
    if (
      node.generated &&
      node.destinationContextKey &&
      node.contextKey !== node.destinationContextKey
    ) {
      // If this is a generated route, we need to get the namedRegex from the original dynamic route
      // but still use the generated route's path for matching
      const originalRoutePath =
        getContextKey(node.destinationContextKey).replace(/\/index$/, '') ?? '/';
      matcher = getNamedRouteRegex(originalRoutePath, absoluteRoute, fileForRegex);
    } else {
      matcher = getNamedRouteRegex(normalizedRoutePath, absoluteRoute, fileForRegex);
    }
